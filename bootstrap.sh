...
# 4. Create shunt scripts for all framework scripts
#    Shunts are real files (not symlinks) so cron jobs can call them.
#    Each shunt sources env.sh then execs the framework canonical version.
#    This means git pull in the framework updates scripts instantly —
#    the shunt just passes arguments through.
echo "[4/5] Creating script shunts..."
mkdir -p "$HERMES_HOME/scripts"

for script in "$FRAMEWORK_DIR/scripts/"*; do
  if [ -f "$script" ]; then
    name="$(basename "$script")"
    shunt="$HERMES_HOME/scripts/$name"
    cat > "$shunt" << SHUNTEOF
#!/usr/bin/env bash
# Shunt — generated by bootstrap.sh. Do not edit.
# Sources env.sh, then execs the canonical framework script.
set -e
...
SHUNTEOF
    chmod +x "$shunt"
    echo "  ✓ scripts/$name (shunt → framework)"
  fi
done

# Symlink overlay env.sh so shunts can source product-specific variables
if [ -f "$OVERLAY_DIR/scripts/env.sh" ]; then
  ln -sf "$OVERLAY_DIR/scripts/env.sh" "$HERMES_HOME/scripts/env.sh"
  echo "  ✓ scripts/env.sh (overlay)"
fi
