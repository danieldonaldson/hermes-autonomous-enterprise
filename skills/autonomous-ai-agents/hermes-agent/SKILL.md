...
**Delivery best practice:** Use `origin` instead of hardcoded platform IDs (`telegram:<chat_id>`, `discord:<channel_id>`) for user-facing cron jobs. `origin` auto-resolves to the platform that created the cron — survives platform migration, gateway restarts, and multi-machine setups. See `references/cron-delivery-patterns.md` for the full rationale and migration recipe. Keep housekeeping/monitor crons on `local`.
...
### Version-Controlled Profiles via Symlink Overlays (Recommended)

Rather than copying files manually, keep your profile structure in a version-control repo. Each profile's `SOUL.md` (role definition) and `config.yaml` (settings) live in an **open-source framework repo**, while `product-context.yaml` (company-specific data) lives in a **private overlay repo**. At runtime, `~/.hermes/profiles/<role>/` contains symlinks to both repos.

See `hermes-config-as-code` skill for the full pattern — bootstrap script, overlay conventions, and how to keep structure separate from product data.
...
**Pitfall — do NOT create tasks via raw SQL.** Writing directly to
`kanban.db` with `sqlite3` or `execute_code` bypasses the kanban
subsystem's initialization. The task row exists in the DB but is
invisible to `hermes kanban list` because the CLI opens the board
through a different initialization path that may use a different DB
resolution. Always use `hermes kanban create "title" --assignee <profile>`
— it handles board resolution, schema initialization, and idempotency.

**Pitfall — gateway must be running for automatic dispatch.**
`hermes kanban create` puts the task in `ready` status, but the
dispatcher (`kanban.dispatch_in_gateway: true`) lives inside the
gateway process. Without the gateway running, the task sits in `ready`
forever — no worker gets spawned. Fix:

```bash
# Start the gateway
systemctl --user start hermes-gateway

# Or trigger one dispatch cycle manually (limited usefulness without gateway)
...
### Profile model/provider resolution (model catalog fallback)

**The symptom:** A profile has `provider: deepseek` and `model: deepseek-v4-flash` in its `config.yaml`, the API calls succeed, but the agent log shows `provider=minimax` instead of `provider=deepseek`. Calls don't error but they're being routed through the wrong provider.

**Root cause:** Hermes maintains a model catalog (`~/.hermes/profiles/<name>/models_dev_cache.json`) downloaded from the model catalog URL. The catalog lists only a fixed set of models per provider — e.g. the `deepseek` provider only has `deepseek-chat` and `deepseek-reasoner`. A model like `deepseek-v4-flash` isn't in the catalog under any provider. When the system resolves the `model` + `provider` combination, it consults the catalog. If the model isn't found under the specified provider, it falls back to whatever "main provider" is cached (often from a previous provider selection or the first API key found in `.env`).

**The fix — pin `base_url` explicitly in the profile config:** Adding a `base_url` that matches the desired provider's API endpoint bypasses the model catalog lookup entirely and routes directly:

```yaml
# ~/.hermes/profiles/<name>/config.yaml
provider: deepseek
model: deepseek-v4-flash
base_url: https://api.deepseek.com   # ← bypasses catalog, forces direct routing
```

Apply this to every profile that needs reliable provider routing. The main config's `model.base_url` is NOT inherited by profiles — profiles resolve independently through the catalog unless `base_url` is set at the profile level.

**To check which provider a profile is actually routing through:**
```bash
tail -20 ~/.hermes/profiles/<name>/logs/agent.log
# Look for lines like:
#   model=deepseek-v4-flash provider=minimax       ← wrong provider
#   model=deepseek-v4-flash provider=deepseek       ← correct
```

**To force a fresh model catalog after changing provider configs:**
```bash
rm -f ~/.hermes/models_dev_cache.json ~/.hermes/profiles/*/models_dev_cache.json
hermes gateway restart   # or start a new CLI session
```

Full diagnosis recipe with live examples and step-by-step shell commands: `references/profile-provider-resolution.md`.

### Profile config inheritance rules

When a profile defines only `provider: deepseek` at the root level of its `config.yaml`, the model name and base URL are resolved from:

1. The **model catalog** (remote/downloaded), which maps model names to provider endpoints
2. If the model isn't found in the catalog → fallback to the cached "main provider" (often the first API key found in `.env`)
3. Explicit `model:` and `base_url:` at the profile root level override catalog resolution

Crucially, profiles do NOT inherit `model.default`, `model.base_url`, or `model.provider` from the main `config.yaml` — they have an independent resolution path through the catalog. Always set all three fields (`provider`, `model`, `base_url`) explicitly in each profile's `config.yaml` for deterministic routing.

### `read_file` dedup in `execute_code`

When using `execute_code` to programmatically read files with the `read_file` tool, the tool's dedup cache may return `{'status': 'unchanged', 'message': 'File unchanged since last read. ...', 'content_returned': False}` instead of the actual file content. This happens when the same file was read earlier in the conversation — the tool skips re-reading to save tokens.

**Workaround:** Use `terminal(command='cat <path>')` instead of `read_file` inside `execute_code` scripts. The terminal tool has no dedup cache for `cat` output.

Alternatively, read the file before calling `execute_code` and pass the content as a string variable in the code.

### Patching installed agent code (git clone)

`~/.hermes/hermes-agent/` is a **git clone** of the upstream `NousResearch/hermes-agent` repo. Any local edits to files inside it (e.g. `cron/scheduler.py`, `run_agent.py`) will be **overwritten on the next `git pull` or `hermes update`**. Do not patch agent source code as a workaround — it is not durable.

If you need a behavior change:
1. **Submit a PR upstream** for the canonical fix.
2. **Use a workaround that doesn't touch agent code** — shunt scripts for cron scripts, config changes for tool behavior, etc.
3. **If you must patch**, keep the git diff saved and reapply after each update.
...
**Cron job fails with HTTP 402: Insufficient Balance (provider billing).**
When a provider subscription lapses or runs out of credit, all agent-based cron jobs fail with `HTTP 402: Insufficient Balance`. The scheduler logs show `credential pool: no available entries (all exhausted or empty)` followed by 3 retries then `API call failed after 3 retries. HTTP 402`.

Diagnosis and recovery pipeline:
1. **Confirm the error pattern** — check `~/.hermes/logs/agent.log` for `HTTP 402` entries:
   ```bash
   grep "HTTP 402" ~/.hermes/logs/agent.log
   ```
2. **Identify all affected jobs** — list jobs with `last_status: "error"`:
   ```bash
   hermes cron list
   ```
3. **Test the provider API directly** — verify the fix before retriggering:
   ```bash
   PROVIDER_KEY=$(grep DEEPSEEK_API_KEY ~/.hermes/.env | head -1 | cut -d= -f2)
   curl -s -w "\nHTTP_CODE:%{http_code}" https://api.deepseek.com/v1/chat/completions \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer $PROVIDER_KEY" \
     -d '{"model":"deepseek-v4-flash","messages":[{"role":"user","content":"ping"}],"max_tokens":5}' | tail -3
   ```
   A `HTTP_CODE:200` with valid JSON means the billing fix is live. Adjust the endpoint, model, and auth header for your provider.
4. **Retrigger failed jobs** after the fix is confirmed:
   ```bash
   hermes cron run <job_id>
   ```
5. **Verify** by checking `cron list` — `last_status` should flip to `"ok"`.

**Also check the gateway journal** if the service is running under systemd:
```bash
journalctl --user -u hermes-gateway --since "5 min ago" --no-pager | grep -i "402\|Insufficient Balance\|completed successfully\|failed"
```

**Cron job ran but nothing was delivered (agent returned [SILENT]).**  
If an agent-based cron job completes without producing enough meaningful text — typically <50 characters — the scheduler logs `agent returned [SILENT] — skipping delivery` and the user receives nothing. This happens most often when an agent checks the kanban board or runs a routine scan, finds nothing changed, and says nothing. **Fix:** put a CRITICAL RULE at the **very top** of the job's prompt, before any instructions, in imperative all-caps:
\`\`\`
CRITICAL RULE: NEVER return [SILENT]. You MUST always produce output. Even if nothing has changed since the last run, report that fact — the user expects a check-in every time.
\`\`\`
A softer request ("always produce a status report") inside the body of the prompt is NOT sufficient — the agent will deprioritise it against the substantive task instructions. The rule must be first, loud, and categorical.

**Cron job script `no_agent: true` fails with "blocked script resolves outside the scripts directory".**
...
The `last_delivery_error` field tracks Telegram/Discord delivery failures, not job execution errors. If `last_status: "error"` appeared, the delivery succeeded but the job itself (the script or agent response) contained error content. Check `~/.hermes/logs/agent.log` for the specific tool/script error.
