...
**Framework scripts are parameterized** — they source `env.sh` from their own directory at runtime:
...
# Link scripts — framework scripts + overlay env.sh
mkdir -p "$HOME/.hermes/scripts"
for script in "$FRAMEWORK_DIR/scripts/"*; do
  ln -sf "$script" "$HOME/.hermes/scripts/$(basename $script)"
done
